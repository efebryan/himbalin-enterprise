...
const Furniture = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getProducts({ category: "Furniture" });
        setProducts(data);
      } catch (err) {
        console.error("Failed to load furniture products:", err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="container mx-auto px-4 md:px-8">
            ...
            {products.length === 0 && !loading ? (
              <div className="text-center py-24">
                <p className="text-gray-400 font-medium">No furniture products found.</p>
              </div>
            ) : (
