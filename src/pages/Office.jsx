...
const Office = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getProducts({ category: "Office" });
        setProducts(data);
      } catch (err) {
        console.error("Failed to load office products:", err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="container mx-auto px-4 md:px-8">
            {products.length === 0 && !loading ? (
              <div className="text-center py-24">
                <p className="text-gray-400 font-medium">No office products found.</p>
              </div>
            ) : (
