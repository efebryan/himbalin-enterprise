...
const ArtificialGrass = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const data = await getProducts({ category: "Artificial Grass" });
        setProducts(data);
      } catch (err) {
        console.error("Failed to load artificial grass products:", err);
      } finally {
        setLoading(false);
      }
    };
          ...
          <div className="container mx-auto px-4 md:px-8">
            {products.length === 0 && !loading ? (
              <div className="text-center py-24">
                <p className="text-gray-400 font-medium">No artificial grass products found.</p>
              </div>
            ) : (
            ...
            {products.length > 0 && (
              <div className="flex flex-col items-center gap-6 pb-12">
                <p className="font-sans text-sm text-gray-400">
                  Showing{" "}
                  <span className="text-himbalin-dark font-medium">{products.length}</span> products
                </p>
              </div>
            )}
          </div>
